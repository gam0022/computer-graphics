/*課題2-2オプション　この中で線分を描く関数linei( )と、円を描く関数circlei( )をそれぞれBresenham, 
  Michenerのアルゴリズムで作成して挿入し、プログラムをコンパイル実行すること。レポート

  は、作成した部分（関数単位）のソースプログラムリスト、実行結果の図、考察を入れること。
  表示ウィンドウをマウスで大きさを変化させたときにも、
  16方向の放射線状の線分とそれを囲む円が描かれるように、
  display( ), reshape( )関数等を変更すること。*/
...
{ 
  ...
  for( i=0; i<16; i++ ){  
    ...
    ix = (int)(r*cos(t)) + width/2;
    iy = (int)(r*sin(t)) + height/2;
    //if(8<=i && i<=12)
    linei( width/2, height/2, ix, iy ); // 中心から放射状に16方向に直線を引く    
  } 
  circlei( width/2,height/2,r); // ウィンドウ幅の直径の円を描く
  glFlush(); 
} 
